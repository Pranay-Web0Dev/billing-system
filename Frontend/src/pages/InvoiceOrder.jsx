...
const InvoiceOrder = () => {
  ...
  // -------------- THE BARCODE SCANNER LOGIC HERE
  ...
  const fetchProductData = async (barcode) => {
    try {
      ...
      if (productData && productData._id) {
        // setItem(productData._id)
        setQuantity(productData.quantity)
        setPrice(productData.saleprice)
        setSelectedProduct(productData);
      } else {
        console.log('Product not found or invalid data');
        setProduct("");
      }
    } catch (error) {
  ...
  const handleBarcodeDetected = (data) => {
    ...
    if (scannedData && !isProductFetched) {
      fetchProductData(scannedData);
  ...
  const handleAddToTable = (e) => {
    ...
    const newData = {
      customerID,
      // productId:  item,
      productId: selectedProduct._id ? selectedProduct._id : item,
      quantity,
